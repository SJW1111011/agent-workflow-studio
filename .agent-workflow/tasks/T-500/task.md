...
- In scope:
  ...
  - repo path: test/server-static-dashboard.test.js
- Out of scope:
  - repo path: dashboard/ (old dashboard preserved as fallback and current feature-complete UI)
...
- lint/test/smoke coverage updated so scaffold validation can coexist with the legacy dashboard fallback
...
- The new shell is intentionally scaffold-only, so day-to-day task operations still depend on the legacy dashboard until migration tasks land
