...
export const EDITABLE_DOCUMENTS = {
  "task.md": {
    ...
    managedSections: ["Heading from task id/title", "Recipe block from task.json and the active recipe"],
    freeSections: ["Goal", "Scope", "Required docs", "Deliverables", "Risks"],
  },
  "context.md": {
    ...
    freeSections: ["Why now", "Facts", "Open questions", "Any extra custom constraints"],
  },
  "verification.md": {
    ...
    managedSections: ["Heading from task id", "Managed verification record block under Evidence after an explicit local refresh"],
    freeSections: ["Draft checks", "Verification records", "Blocking gaps", "Any manual notes between evidence refreshes"],
  },
};

export function parseRunEvidenceDraft(values = {}) {
...
export function parseRunChecks(text, runStatus) {
...
export function parseRunCheckLine(line, fallbackStatus) {
...
export function parseRepoRelativeList(text) {
  return uniqueStrings(parseLooseList(text));
}

export function getPendingProofPaths(detail) {
...
export function buildPendingProofCheckLines(detail) {
  return getPendingProofPaths(detail).map((item) => `Review ${item} diff`);
}

export function mergeProofPathDraft(currentText, detail) {
  return uniqueStrings(parseRepoRelativeList(currentText).concat(getPendingProofPaths(detail))).join("\n");
}

export function mergeProofCheckDraft(currentText, detail, runStatus) {
...
export function buildVerificationProofDraft(detail, currentText = "") {
  return buildVerificationProofDraftFromPaths(getPendingProofPaths(detail), currentText);
}

export function buildVerificationProofDraftFromPaths(paths, currentText = "") {
...
export function buildVerificationPlannedCheckDraft(detail, currentText = "") {
...
export function parseRunVerificationDraft(values = {}) {
  const status = String(values.status || "draft").trim().toLowerCase();
  return {
    status,
...
export function buildVerificationPlannedCheckDraftFromRunDraft(values = {}, currentText = "") {
  const existingChecks = extractVerificationPlannedManualChecks(currentText);
...
export function formatVerificationPlannedCheck(check) {
...
export function mergeVerificationPlannedCheckDraft(currentText, detail) {
...
function mergeVerificationPlannedCheckLines(currentText, draftLines) {
  ...
  const plannedRange =
    findMarkdownSectionRange(normalized, "Draft checks") || findMarkdownSectionRange(normalized, "Planned checks");
  if (plannedRange) {
...
export function mergeVerificationProofDraft(currentText, detail) {
...
export function mergeVerificationProofPlanDraft(currentText, detail) {
  return mergeVerificationProofDraft(mergeVerificationPlannedCheckDraft(currentText, detail), detail);
}

export function mergeVerificationFromRunDraft(currentText, values = {}) {
  const normalized = String(currentText || "").replace(/\r\n/g, "\n").trim();
  const runDraft = parseRunVerificationDraft(values);
...
export function hasRunDraftVerificationContent(values = {}) {
  const runDraft = parseRunVerificationDraft(values);
  return runDraft.scopeProofPaths.length > 0 || runDraft.verificationChecks.length > 0;
}

export function extractVerificationProofPaths(text) {
  const proofSection =
...
export function extractVerificationPlannedManualChecks(text) {
...
export function getNextProofNumber(text) {
...
export function parseLooseList(text) {
  return String(text || "")
    .split(/\r?\n/)
...
export function splitLineEntries(text) {
  return String(text || "")
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
...
export function extractVerificationPlannedChecks(text) {
...
export function normalizeVerificationPlannedCheckLine(line) {
  const normalized = String(line || "").replace(/^\s*[-*+]\s*/, "").trim();
  if (!normalized) {
    return "";
  }
...
export function describeVerificationProofSignals(verificationGate, verificationText = "") {
...
export function getEditableDocumentConfig(documentName) {
  return EDITABLE_DOCUMENTS[documentName] || EDITABLE_DOCUMENTS["task.md"];
}

export function getEditableDocumentContent(documentName, content = "") {
...
export function stripManagedVerificationAnchorBlock(content = "") {
...
export function hasManagedVerificationAnchorBlock(content = "") {
